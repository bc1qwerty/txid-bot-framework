...
//
// ⚠**같은 결함을 이미 한 번 고쳤는데 또 났다.** `pkg/notify` 의 `scrubToken`(2026-08-29,
// fb84d29)은 **우리 코드가 반환·기록하는 err** 를 가린다. 그런데 tgbotapi 는 자기
// 고루틴에서 `log.Println(err)` 로 직접 찍기 때문에(`bot.go:445`, getUpdates 재시도 루프)
// 우리 scrub 을 한 번도 거치지 않는다. 둘은 상보적이다 — 어느 쪽도 지우지 말 것.
// 교훈: **라이브러리가 스스로 찍는 로그는 우리 오류 처리 경로 밖에 있다.**
//
